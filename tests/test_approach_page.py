...
def approach_at() -> AppTest:
    """Every test below except the reveal-button one only reads the default
    render -- one shared cold start instead of 14 redundant ones. The
    reveal-button test clicks a widget and reruns, so it keeps its own
    fresh instance (sharing would leak that click into every other test)."""
    return _run_approach()


def test_approach_page_runs_without_exceptions(approach_at):
    assert not approach_at.exception


def test_approach_page_has_baseline_and_all_five_steps(approach_at):
    header_texts = [h.value for h in approach_at.header]
    for expected in [
...
def test_approach_page_baseline_discloses_no_collaborative_filtering(approach_at):
    info_texts = " ".join(i.value for i in approach_at.info)
    assert "collaborative filtering isn't attempted here" in info_texts.lower()


def test_approach_page_step1_offers_real_playable_window_clips(approach_at):
    ...
    body_texts = " ".join(c.value for c in approach_at.caption) + " ".join(m.value for m in approach_at.markdown)
    assert "5" in body_texts  # WINDOW_SEC=5.0 -- loose text check, exact value asserted in unit tests
    assert len(approach_at.get("audio")) >= 4  # at least the window clips themselves


def test_approach_page_step2_shows_all_seven_facets_with_descriptions(approach_at):
    caption_texts = " ".join(c.value for c in approach_at.caption)
...
def test_approach_page_step2_harmony_shows_estimated_key_and_chord_strip(approach_at):
    ...
    body_text = " ".join(m.value for m in approach_at.markdown) + " ".join(c.value for c in approach_at.caption)
    assert "estimated key" in body_text.lower()
    assert "krumhansl-schmuckler" in body_text.lower()


def test_approach_page_step2_harmony_seek_uses_the_chord_strip_not_the_heatmap(approach_at):
...
def test_approach_page_step2_sound_shows_mel_spectrogram_as_supporting_visual_only(approach_at):
...
def test_approach_page_step2_stem_facets_use_real_cdoc_stems(approach_at):
...
def test_approach_page_step2_sound_tags_shows_tag_list_not_waveform(approach_at):
...
def test_approach_page_step3_has_embedding_strip_and_close_by_illustration(approach_at):
    caption_texts = " ".join(c.value for c in approach_at.caption)
    assert "512-dimensional space" in caption_texts
    assert "illustrative, not real data" in caption_texts.lower()


def test_approach_page_step4_has_song_dna_and_states_the_averaging_limitation(approach_at):
    warning_texts = " ".join(w.value for w in approach_at.warning)
    assert "computed once per whole song, not per moment" in warning_texts
    assert "quiet intro and a loud chorus" in warning_texts


def test_approach_page_step5_shows_expandable_real_tags(approach_at):
    expander_labels = [e.label for e in approach_at.expander]
...
def test_approach_page_reveal_button_does_not_crash():
    ...
    configured API key -- clicking it must never raise. Needs its own fresh
    instance (not the shared approach_at fixture) since it clicks a widget
    and reruns."""
    at = _run_approach()
...
def test_approach_page_demo_song_is_pinned_to_freak_of_nature(approach_at):
