...
def top1_score_distribution(
    ...
    for seg_id in query_seg_ids:
        ...
        # top-1: nearest real neighbor from a different song, excluding the query's own song
        raw = embedding_repo.search(facet_name, query_vec, k=20)
        neighbor_score = next(
            (score for cand_id, score in raw if segment_song.get(cand_id) not in (None, song_id)), None
        )
        if neighbor_score is not None:
            top1_scores.append(float(neighbor_score))
