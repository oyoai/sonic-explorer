...
st.write(
    "**Methodology** and **Results** covered how the library was analyzed, improved, and evaluated. "
    "This page is different: it's a guided tour of the live app itself -- what you're actually "
    "looking at on each screen, how to read it, and what the shapes/colors mean -- using the real, "
    "currently-running components, not screenshots."
)
st.page_link("pages/2_Results.py", label="← Back to Results", icon="\U0001F4CA")
show_logo()
...
- **Position** here comes from PCA -- the two directions of maximum spread (variance) across the
  ...
  *nameable*: the live page's "Inspect these axes" expander runs a real correlation check against
  tempo/energy/brightness/harmonic-complexity/rhythmic-density first (see Methodology §6b for what
...
""")
st.page_link("pages/7_Explore.py", label="Open Explore →", icon="\U0001F310")
...
""")
st.page_link("pages/4_Song_XRay.py", label="Open Song X-Ray →", icon="\U0001F50D")
...
""")
st.page_link("pages/5_Moment_Matcher.py", label="Open Moment Matcher →", icon="\U0001F3AF")
...
st.page_link("pages/6_Ask_The_DJ.py", label="Open Ask the DJ →", icon="\U0001F399️")
...
with st.expander("Direct links to the other states (for this walkthrough only)"):
    ...
    with cta_cols[0]:
        st.page_link("pages/4_Song_XRay.py", label="**Song X-Ray**", icon="\U0001F50D")
        st.caption("A song's structural anatomy, fingerprints, and DNA in one place.")
    with cta_cols[1]:
        st.page_link("pages/5_Moment_Matcher.py", label="**Moment Matcher**", icon="\U0001F3AF")
        st.caption("Pick a moment, get ranked matches on any facet, with explanations.")
    with cta_cols[2]:
        st.page_link("pages/6_Ask_The_DJ.py", label="**Ask the DJ**", icon="\U0001F399️")
